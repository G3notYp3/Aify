...
function showPost() {
    global $bdd;
    if(!empty($_SESSION['ID'])) {
        $post_infos = $bdd->prepare('SELECT posts.*, posts.ID AS pID, users.ID AS uID, user_name, user_sname FROM posts INNER JOIN users ON posts.post_author = users.ID WHERE users.ID = :id ORDER BY posts.ID DESC LIMIT 0, 5');
...
?>
                    ...
                    <br>
<?php
                    echo '<time>' . $post_infos_result['post_date'] . '</time>';
?>
                </header>
